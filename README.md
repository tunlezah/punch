...
- **Edit freely:** dates, `Type`, the four times, `Notes`, everything in *Leave and travel*, *TOIL adjustments* and *Public holidays*, and the `Config` block.
...
- **Leave / TDY / Public holiday** days are credited the standard day (0 on non-working days) unless a per-day figure is set. Spans in *Leave and travel* and dates in *Public holidays* generate these days; only days you edit individually appear as rows.
- **Public holidays are flexible.** A date in the *Public holidays* table is treated as a public holiday by default, but only when that day has no record of its own — clock in, mark it Sick, or set its type in Recent and the record wins. The list itself is yours to edit: change a date to move a holiday your workplace observes on another day, rename it, delete one you don't get, or add a local show day. Picking a jurisdiction in Settings loads the official 2026–2027 dates into the table (adding only dates not already listed, so your edits survive a reload).
...
- **Days generated by spans and by the public-holiday list are not written as day rows.** The *Leave and travel* and *Public holidays* tables are the source of truth; period totals and balances in the file include those days. To override one day, give it a row (the UI does this when you edit such a day in Recent). This keeps `parse(render(state)) ≡ state` exact and makes changing a span's hours or moving a holiday take effect everywhere.
...
- **Public holidays** for 2026 and 2027 are bundled per jurisdiction (national plus ACT, NSW, NT, QLD, SA, TAS, VIC, WA) and copied into the file's *Public holidays* table when you pick a jurisdiction, so the dates you actually observe are visible and editable rather than locked inside the app. `holiday_region` only remembers which list was loaded. Part-day holidays (Christmas Eve / New Year's Eve evenings), the NSW Bank Holiday (banks only) and regional show days are not in the bundled lists — add the ones you get. See *Public holiday sources* below.
...
The bundled dates in `public/js/holidays.js` follow the Fair Work Ombudsman's [public holidays](https://www.fairwork.gov.au/employment-conditions/public-holidays) pages for 2026 and 2027, which in turn list each state and territory government's gazetted dates. Both year pages were fetched on 6 September 2026 (Fair Work's own "content last updated" stamps: 7 August 2026 for the 2026 page, 31 August 2026 for the 2027 page) and the bundled lists reproduce them per jurisdiction, with these deliberate omissions: part-day holidays (Christmas Eve and New Year's Eve evenings in the NT, Queensland and SA); holidays limited to an area or a workforce (Royal Queensland Show — Brisbane only; Royal Hobart Regatta, Royal Hobart Show and Recreation Day — parts of Tasmania; Easter Tuesday — Tasmanian public service); and Victoria's 2027 Friday before the AFL Grand Final, which Fair Work lists as date TBC. The NSW Bank Holiday applies to banks only and is not listed. Add any of these that apply to you with the Add row in Settings. Notable 2026–2027 facts the lists carry: the ACT, NSW and WA add a Monday when Anzac Day falls on a weekend, Queensland and the NT move it to the Monday in 2027, SA/Tasmania/Victoria do not; Tasmania observes Boxing Day on the Monday (2026) and Tuesday (2027) only; SA's 26 December holiday is Proclamation Day.

Because the list lives in the file and is editable, a wrong or changed date is a one-cell fix rather than an app update.
...
public/            the app (served as-is)
  ...
  js/holidays.js   bundled Australian public holidays 2026–2027 (loaded into the file on request)
