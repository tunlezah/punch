...
import { DAY_NAMES, DAY_NAMES_LONG, fmtDateAU, parseDuration, fmtDuration, fmtDelta, normHHMM, isISODate, fmtClock } from '../time.js';
import { ACCENTS, THEMES, ROUNDING_OPTIONS, periodFor, weeklyStandardMinutes, firstDataDate } from '../calc.js';
import { REGIONS, holidayList } from '../holidays.js';
...
export function renderSettings(root, app) {
  ...
  // ---- public holidays ---------------------------------------------------------
  const region = h('select', { 'aria-label': 'Public holiday region' }, REGIONS.map((r) => h('option', { value: r.code, selected: r.code === cfg.holidayRegion }, r.name)));
  const regionHint = h('div', { class: 'setting-help' });
  const refreshRegion = () => {
    const c = store.state.config;
    const n = holidayList(c.holidayRegion).length;
    regionHint.textContent = c.holidayRegion === 'none' ? 'Public holidays are only recorded when you press the Public holiday button.' : `${n} dates for 2026–2027 are pre-marked as public holidays (credited a standard day). Give a day its own record to override.`;
  };
  region.addEventListener('change', () => { set({ holidayRegion: region.value }, 'Public holiday region'); refreshRegion(); });
  refreshRegion();
  ...
  root.append(
    h('div', { class: 'settings-grid' },
      ...
      h('section', { class: 'card', 'aria-labelledby': 's-ph' },
        h('div', { class: 'card-head' }, h('h3', { id: 's-ph' }, icon('flag', 16), 'Public holidays')),
        setting('Jurisdiction', 'Pre-marks national and state/territory public holidays for 2026 and 2027.', region),
        h('div', { class: 'setting' }, regionHint),
      ),
